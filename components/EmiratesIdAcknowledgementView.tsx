...
import { Employee, JobOffer, EmiratesIdAcknowledgement } from '../types';
import { 
  Search, FileText, FileDown, Calendar, User, 
  ChevronRight, CreditCard, ShieldCheck, Trash2, PlusCircle, Check
} from 'lucide-react';
import { handleFirestoreError, OperationType } from '../services/storageService';

export const downloadEmiratesIdAcknowledgementPDF = (record: EmiratesIdAcknowledgement) => {
    const doc = new jsPDF({
    ...
    doc.text("Pioneer Document Management Work Portal — ID Card Management Division", 20, 30);
    ...
    doc.text(`Ref No: ${record.refNo || 'PGC/HR/EID/TEMP'}`, 20, 52);
    ...
    const headline = "EMIRATES ID COLLECTION ACKNOWLEDGEMENT";
    ...
    const bodyText = "This is to confirm and acknowledge that the Company has handed over, and the below-mentioned Employee has safely received and taken collection of, their original physical Emirates ID card after successful visa residency processing.";
    ...
    const tableData = [
        ...
        { label: "Emirates ID Number", value: record.emiratesId },
        { label: "Nationality", value: record.nationality },
        { label: "Date of Handover", value: formattedIssueDate },
        { label: "Purpose / Event", value: record.purpose }
    ];
    ...
    doc.setFontSize(10);
    doc.setTextColor(71, 85, 105); // Slate-600
    const safeNote = "The Employee acknowledges receipt of their physical Emirates ID card and takes full personal responsibility for its safe custody, usage, and compliance with UAE federal laws and regulations.";
    ...
    doc.save(`EmiratesID_Acknowledgement_${record.employeeName.replace(/\s+/g, '_')}.pdf`);
};

export const EmiratesIdAcknowledgementView: React.FC<{
  employees: Employee[];
}> = ({ employees }) => {
    // Realtime collections state
    const [jobOffers, setJobOffers] = useState<JobOffer[]>([]);
    const [savedRecords, setSavedRecords] = useState<EmiratesIdAcknowledgement[]>([]);
    ...
    const [purpose, setPurpose] = useState<string>('Handover of physical processed Emirates ID Card to employee');
    ...
    useEffect(() => {
        ...
        const unsubOffers = onSnapshot(qOffers, (snap) => {
            ...
            handleFirestoreError(err, OperationType.GET, 'job_offers');
        });

        // 2. Subscribe to Saved Acknowledgements
        const qEID = query(collection(db, 'emirates_id_acknowledgements'), orderBy('createdAt', 'desc'));
        const unsubEID = onSnapshot(qEID, (snap) => {
            const list: EmiratesIdAcknowledgement[] = [];
            snap.forEach((d) => {
                list.push({ ...d.data(), id: d.id } as EmiratesIdAcknowledgement);
            });
            setSavedRecords(list);
            setLoading(false);
        }, (err) => {
            handleFirestoreError(err, OperationType.GET, 'emirates_id_acknowledgements');
            setLoading(false);
        });
    ...
    const filteredEmployees = useMemo(() => {
        ...
        const matchEmps = employees.filter(emp => 
            ...
            (emp.documents?.emiratesId && emp.documents.emiratesId.toLowerCase().includes(term))
        ).map(emp => ({
            id: emp.id,
            name: emp.name,
            code: emp.code,
            type: 'employee' as const,
            emiratesId: emp.documents?.emiratesId || '',
        ...
        const matchOffers = jobOffers.filter(offer => 
            ...
            emiratesId: '', // Usually empty until cards are generated
            nationality: '',
    ...
    const handleSelectEntity = (ent: {
        ...
        setRefNo(`PGC/HR/EID/${initials}${dateObj.getFullYear().toString().slice(-2)}${String(dateObj.getMonth()+1).padStart(2,'0')}-${randNum}`);
        setSearchTerm('');
    ...
    const handleStartManual = () => {
        ...
        setRefNo(`PGC/HR/EID/M${Math.floor(100 + Math.random() * 900)}-${Date.now().toString().slice(-4)}`);
        setActiveSavedId(null);
    };

    // Load saved record for editing/preview
    const handleLoadSavedRecord = (rec: EmiratesIdAcknowledgement) => {
    ...
    const handleSaveRecord = async () => {
        if (!employeeName.trim() || !emiratesId.trim() || !nationality.trim()) {
            setErrorMsg("Employee Name, Emirates ID, and Nationality are absolutely necessary to validate receipt.");
        ...
        const recordId = activeSavedId || `eida_${Date.now().toString(36)}_${Math.random().toString(36).substr(2, 4)}`;
        
        const finalRecord: EmiratesIdAcknowledgement = {
            id: recordId,
            refNo: refNo || `PGC/HR/EID-${Math.floor(1000 + Math.random() * 9000)}`,
            date,
            employeeId,
            employeeName,
            emiratesId,
        ...
        try {
            await setDoc(doc(db, 'emirates_id_acknowledgements', recordId), finalRecord);
            setSuccessMsg(`Handover acknowledgement for ${employeeName} saved successfully!`);
            setActiveSavedId(recordId);
            setTimeout(() => setSuccessMsg(null), 4000);
        } catch (err) {
            handleFirestoreError(err, OperationType.WRITE, `emirates_id_acknowledgements/${recordId}`);
    ...
    const handleDeleteRecord = async (id: string) => {
        try {
            await deleteDoc(doc(db, 'emirates_id_acknowledgements', id));
            if (activeSavedId === id) {
                // reset fields
                setActiveSavedId(null);
                ...
                setEmiratesId('');
                setNationality('');
                setPurpose('Handover of physical processed Emirates ID Card to employee');
            }
            setDeleteConfirmId(null);
            setSuccessMsg("EMIRATES ID HANDOVER REJECTED/DISCARDED SUCCESSFULLY.");
            setTimeout(() => setSuccessMsg(null), 3000);
        } catch (err) {
            handleFirestoreError(err, OperationType.DELETE, `emirates_id_acknowledgements/${id}`);
    ...
    // Trigger printed copy directly
    const handleDirectPrintRecord = () => {
        const recordMock: EmiratesIdAcknowledgement = {
            id: activeSavedId || 'temp',
            refNo: refNo || 'PGC/HR/EID-TEMP',
            date,
            employeeId,
            employeeName,
            emiratesId,
        ...
        downloadEmiratesIdAcknowledgementPDF(recordMock);
    };
    ...
    const filteredSavedRecords = useMemo(() => {
        ...
        return savedRecords.filter(rec => 
            ...
            rec.emiratesId.toLowerCase().includes(queryVal) ||
    ...
    return (
        <div id="emirates-id-ack-view" className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-4 mb-2">
                <div className="space-y-1">
                    ...
                    <h1 className="text-2xl font-black text-slate-800 flex items-center gap-2">
                        <CreditCard className="w-6.5 h-6.5 text-indigo-600" /> Emirates ID Collection Acknowledgement
                    </h1>
                    <p className="text-sm text-slate-500">
                        Record secure handover and generate legitimate formal Emirates ID Collection/Handover Acknowledgements for processed employees and onboarding job candidates.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                {/* 1. Left Selection Column (4 cols) */}
                <div className="lg:col-span-4 space-y-5">
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                        <h3 className="text-xs font-extrabold text-slate-700 uppercase tracking-wider flex items-center gap-2">
                            <User className="w-4 h-4 text-slate-400" /> Search Candidate / Employee
                        </h3>
                        ...
                        {selectedEntity && (
                            <div className="p-4 bg-indigo-50/60 border border-indigo-100/50 rounded-2xl space-y-2">
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <span className="text-xs font-extrabold text-slate-700">Active Handover Draft</span>
                                </div>
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                        <div className="flex items-center justify-between">
                            <h3 className="text-xs font-extrabold text-slate-700 uppercase tracking-wider flex items-center gap-2">
                                <FileText className="w-4 h-4 text-indigo-500" /> Handover Directory ({savedRecords.length})
                            </h3>
                        </div>
                        ...
                        <div className="relative">
                            ...
                            <input 
                                ...
                                placeholder="Filter saved handover files..."
                        ...
                        {loading ? (
                            ...
                            <p className="text-center py-6 text-xs text-slate-400 italic">No saved acknowledgments found.</p>
                        ) : (
                                            ...
                                            {deleteConfirmId === rec.id ? (
                                                ...
                                                <>
                                                    <button 
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            downloadEmiratesIdAcknowledgementPDF(rec);
                                                        }}
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                        ...
                        {selectedEntity ? (
                            <div className="space-y-3.5 text-xs">
                                ...
                                <div>
                                    <label className="block text-slate-400 font-extrabold mb-1">Handover Date</label>
                                    <input 
                                        type="date"
                                        className="w-full p-2.5 border border-slate-200 rounded-lg font-bold text-slate-800 focus:ring-1 focus:ring-indigo-500 outline-none"
                                        value={date}
                                ...
                                <div>
                                    <label className="block text-slate-400 font-extrabold mb-1">Emirates ID Number</label>
                                    <input 
                                        type="text"
                                        className="w-full p-2.5 border border-slate-200 rounded-lg font-bold text-slate-800 focus:ring-1 focus:ring-indigo-500 outline-none"
                                        value={emiratesId}
                                        onChange={e => setEmiratesId(e.target.value)}
                                        placeholder="e.g. 784-1992-1234567-1"
                                    />
                                </div>
                                ...
                                <div>
                                    <label className="block text-slate-400 font-extrabold mb-1">Handover Purpose / Reason</label>
                                    <textarea 
                                ...
                                <div>
                                    <label className="block text-slate-400 font-extrabold mb-1">Company Representative Author</label>
                                    <input 
                                        type="text"
                                        className="w-full p-2.5 border border-slate-200 rounded-lg font-bold text-slate-800 focus:ring-1 focus:ring-indigo-500 outline-none"
                                        value={signatoryName}
                                        onChange={e => setSignatoryName(e.target.value)}
                                        placeholder="Name of who hands over EID"
                                    />
                                </div>

                                <div>
                                    <label className="block text-slate-400 font-extrabold mb-1">Company Representative Title</label>
                                    <input 
                                        type="text"
                                        className="w-full p-2.5 border border-slate-200 rounded-lg font-bold text-slate-800 focus:ring-1 focus:ring-indigo-500 outline-none"
                ...
                <div className="lg:col-span-4 bg-slate-100/50 p-6 rounded-3xl border border-slate-200/60 shadow-inner flex justify-center">
                    ...
                    <div id="interactive-sheet" className="w-[100%] shadow-lg border border-slate-300 rounded-xs bg-white p-5 text-slate-800 select-none relative overflow-hidden font-sans space-y-4" style={{ minHeight: '600px', fontSize: '11px' }}>
                        ...
                        <div className="border-b-2 border-slate-200 pb-3 flex flex-col gap-0.5">
                            ...
                            <p className="text-[7.5px] font-bold text-slate-400">Pioneer DMS • Official ID Card Management Department</p>
                            
                            <div className="flex justify-between items-center text-[8px] text-slate-500 pt-1.5 font-mono">
                                <span className="font-bold">Ref No: {refNo || "PGC/HR/EID/TEMP"}</span>
                        ...
                        <div className="text-center py-1">
                            <h3 className="text-[11px] font-black tracking-wide text-slate-900 uppercase underline text-center decoration-1 underline-offset-4 decoration-slate-900">
                                EMIRATES ID COLLECTION ACKNOWLEDGEMENT
                            </h3>
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-slate-600 font-sans leading-relaxed text-[8.5px]">
                                This is to confirm and acknowledge that the Company has handed over, and the below-mentioned Employee has safely received and taken collection of, their original physical Emirates ID card after successful visa residency processing.
                            </p>
                        </div>
                        ...
                        <div className="w-full border border-slate-800 divide-y divide-slate-800 rounded-sm overflow-hidden text-[8px]">
                            ...
                            <div className="flex divide-x divide-slate-800">
                                <div className="w-[35%] bg-slate-50 p-1.5 font-bold text-slate-700">Emirates ID Number</div>
                                <div className="w-[65%] p-1.5 font-mono font-bold text-slate-900">{emiratesId || "784-1992-1234567-1"}</div>
                            </div>
                            <div className="flex divide-x divide-slate-800">
                            ...
                            <div className="flex divide-x divide-slate-800">
                                <div className="w-[35%] bg-slate-50 p-1.5 font-bold text-slate-700">Date of Handover</div>
                                <div className="w-[65%] p-1.5 font-mono text-slate-900">
                                    {new Date(date).toLocaleDateString('en-GB')}
                                </div>
                            </div>
                            <div className="flex divide-x divide-slate-800">
                                <div className="w-[35%] bg-slate-50 p-1.5 font-bold text-slate-700">Purpose / Event</div>
                        ...
                        <p className="text-[8px] text-slate-500 font-sans leading-relaxed">
                            The Employee acknowledges receipt of their physical Emirates ID card and takes full personal responsibility for its safe custody, usage, and compliance with UAE federal laws and regulations.
                        </p>
