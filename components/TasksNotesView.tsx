...
export default function TasksNotesView({ systemUser }: TasksNotesViewProps) {
  ...
  const handleSaveMeeting = async (e: React.FormEvent) => {
    ...
    const meetingData: Partial<Meeting> = {
      ...
      completed: showMeetingForm.completed || false,
      updatedAt: new Date().toISOString()
    };
  ...
  const filteredMeetings = useMemo(() => {
    return meetings.filter(m => {
      ...
      const matchesFilter = meetingFilter === 'completed' ? m.completed === true : m.completed !== true;

      return matchesSearch && belongsToUser && matchesFilter;
    });
  }, [meetings, meetingSearch, meetingFilter, isEmployee, systemUser]);
  ...
  }, [meetings, isEmployee, systemUser]);
      ...
      {activeSubTab === 'meetings' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-3xl border border-slate-100 shadow-sm">
              ...
              <div className="flex bg-slate-50 p-1 rounded-2xl border border-slate-100">
                ...
                >
                  ...
                  <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-black ${
                    meetingFilter === 'scheduled' ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-200 text-slate-500'
                  }`}>
                    {meetingStats.scheduledCount}
                  </span>
                </button>
                <button
                ...
                >
                  ...
                  Completed
                  <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-black ${
                    meetingFilter === 'completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-200 text-slate-500'
                  }`}>
            ...
            {canScheduleMeetings && (
              <button
                onClick={() => setShowMeetingForm({ duration: 30, dateTime: new Date().toISOString().slice(0, 16), completed: false })}
          ...
          {loadingMeetings ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 bg-white border border-slate-100 rounded-[2rem] shadow-sm text-center px-6">
              ...
              <h3 className="text-slate-700 font-bold text-base mt-4">
                {meetingFilter === 'completed' ? 'No Completed Meetings' : 'No Meetings Scheduled'}
              </h3>
              <p className="text-slate-400 text-xs mt-1 max-w-sm">
                {meetingFilter === 'completed' 
                  ? 'No finished sessions could be found matching your search query.' 
                  : 'No synchronized team sessions or Google Meet sessions could be found matching your search query.'}
              </p>
              {canScheduleMeetings && meetingFilter !== 'completed' && (
                <button
                  onClick={() => setShowMeetingForm({ duration: 30, dateTime: new Date().toISOString().slice(0, 16), completed: false })}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden border border-slate-100 max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleSaveMeeting} className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              {/* Meeting Completion Toggle */}
              <div className="space-y-1.5 pt-2 border-t border-slate-100">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block text-left">Meeting Status</label>
                <label className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-100/80 rounded-2xl cursor-pointer hover:bg-slate-100/50 transition-all">
                  <input
                    type="checkbox"
                    checked={showMeetingForm.completed || false}
                    onChange={(e) => setShowMeetingForm({ ...showMeetingForm, completed: e.target.checked })}
                    className="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                  />
                  <div className="flex flex-col text-left">
                    <span className="text-xs font-black text-slate-800">Mark as Completed</span>
                    <span className="text-[10px] font-semibold text-slate-400 mt-0.5">This removes the active meeting banner alert and badges the card as completed.</span>
                  </div>
                </label>
              </div>
